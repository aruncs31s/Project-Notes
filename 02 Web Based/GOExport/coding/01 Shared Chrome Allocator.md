...
> **Update (2026-08-05):** Added `chromedp.Env` variable overrides pointing `XDG_CONFIG_HOME`, `XDG_CACHE_HOME`, `XDG_DATA_HOME`, and `HOME` to the temporary data directory, preventing `chrome_crashpad_handler` from crashing due to permission issues when starting Chrome inside restricted environments like Render.
